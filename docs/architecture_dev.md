...
This guide describes how QEC developers can contribute with new architectures
to Guppy FT.
...
In this page, we refer to the *computational*, *logical* and *physical* abstraction
...
| Logical API | Application & QEC co-design | Formalise the architecture's gate set |
...
the Steane $[[7,1,3]]$ code, linking to the relevant source in our repository.
...
The guidelines laid out in this page are meant to promote the adoption of a
...
primitives together, and easily swap out with alternative implementations.
Composition of primitives into logical gadgets &mdash;such as a $T$ gate&mdash;
belong to the [logical API](#logical-api) as convenience functions.

When applying measurements, be mindful of where you use `read()`.
...
we wrap the logical block into another Guppy struct, `PreBlock` (see [source code](https://github.com/quantinuum-dev/guppyft/blob/main/src/guppyft/code/_state_factory.py)),
that contains the array of `Measurement` and a `force_check()` method which
...
* Developing compilation and optimisation passes that transform logical programs.
* Support for global management of resources, such as state factories.
* Writing programs at the logical level, which can then take advantage
of the above features.

The steps to define a logical API are the following.
...
4. Define an architecture builder, following the pattern of `SteaneBuilder` from [`encode.py`](https://github.com/quantinuum-dev/guppyft/blob/main/src/guppyft/code/steane/encode.py).
  * You do not need to provide a `logical_compiler` to `_gen_encoder_spec`; that is
  ...
  * The key contribution is `_gen_implement_spec`. Using the `STATE` class is optional;
   this provides a global context for resource management.
...
ops and types, as illustrated in the "Writing Logical Programs" notebook
(**TODO**: provide link).
Step 3 and 4 provide the `implement_ops` method showcased in the same
notebook; this replaces the opaque logical operations with their physical implementation
...
upon which we can define compilation and optimization passes and global management
...
This abstraction lets developers provide a public interface for the logical ops,
...
You may provide convenience functions in [`logical.py`](https://github.com/quantinuum-dev/guppyft/blob/main/src/guppyft/code/steane/logical.py), such as in the case of the $T$ gate from the Steane architecture reference.
We suggest these convenience functions are not given HUGR ops themselves; the
HUGR extension should only include the fundamental building blocks of the architecture.

We recommend using [semver](https://semver.org/) convention for the versioning of
the HUGR extension.


## End-to-end encoding

This enables any user to automatically encode their Guppy programs using the
...
uses a `ReplacementCompiler` that can replace individual HUGR op nodes with a
specified logical HUGR op &mdash;from your logical API&mdash; or (the compiled HUGR of) a Guppy
...
Ideally, the logical compiler you provide supports any quantum operation offered
in [`guppylang.std.quantum`](https://docs.quantinuum.com/guppy/api/generated/guppylang.std.quantum.html#module-guppylang.std.quantum).
Naturally, you may choose to support only a subset of these. The encoding pass will fail on
user programs outside of this subset.
...
with angles decided at runtime, following https://arxiv.org/pdf/2404.05618.
Architectures where each code block contains multiple logical qubits can
support arbitrary addressability by tracking the qubit assignment at runtime
...
performance on the quantum device if it causes the stalling.
